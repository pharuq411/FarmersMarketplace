...
router.post('/', auth, upload.single('file'), async (req, res) => {
  if (req.user.role !== 'farmer') return err(res, 403, 'Only farmers can upload products', 'forbidden');
  ...
  try {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      try {
        await db.query(
          'INSERT INTO products (farmer_id, name, description, price, quantity, unit, category) VALUES ($1,$2,$3,$4,$5,$6,$7)',
          [req.user.id, row.name.trim(), row.description?.trim() || null, price, quantity,
           row.unit?.trim() || 'unit', row.category?.trim() || 'other']
        );
        results.created++;
      } catch (e) {
        results.skipped++;
    ...
    res.json({ success: true, created: results.created, skipped: results.skipped, errors: results.errors });
  } catch (e) {
